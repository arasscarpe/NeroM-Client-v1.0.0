...
def action_right(hold=0.1):
    pyautogui.mouseDown(button='right')
    time.sleep(hold)
    pyautogui.mouseUp(button='right')

def action_left(hold=0.03):
...
def on_key_press(key):
    try:
        if hasattr(key, 'char') and key.char is not None: k = key.char.lower()
        else: k = str(key).replace("Key.", "").lower()
        if k == SETTINGS["toggle_key"].lower(): comm.toggle_visibility.emit(); return
        for name, data in MACRO_DATA.items():
            if data["active"] and data["bind"] == k:
                threading.Thread(target=run_macro, args=(name,), daemon=True).start()
    except: pass
...
class MacroCard(QFrame):
    def __init__(self, name, code, desc, fields):
        ...
        tv = QVBoxLayout(); tv.addWidget(QLabel(name, styleSheet="color:white; font-weight:bold; font-size: 11px; border:none;")); tv.addWidget(QLabel(desc, styleSheet="color:#555; font-size: 8px; border:none;"))
        ...
        grid.addWidget(QLabel("Key", styleSheet="color:#888; border:none;"), 0, 0); grid.addWidget(ki, 0, 1)
        di = QLineEdit(str(MACRO_DATA[self.name]["delay"])); di.setObjectName("InputBox"); di.setFixedWidth(45)
        di.textChanged.connect(lambda t: MACRO_DATA[self.name].update({"delay": int(t) if t.isdigit() else 80}))
        grid.addWidget(QLabel("Ms", styleSheet="color:#888; border:none;"), 0, 2); grid.addWidget(di, 0, 3)
        for i, f in enumerate(fields):
            ...
            grid.addWidget(QLabel(f, styleSheet="color:#888; border:none;"), r, c); grid.addWidget(inp, r, c + 1)
        lay.addLayout(grid); comm.theme_changed.connect(self.refresh_theme); self.refresh_theme()
    
    def update_state(self, s): 
        MACRO_DATA[self.name]["active"] = s
        self.refresh_theme()

    def refresh_theme(self):
        theme = THEME_MAP[SETTINGS["current_theme"]]
        self.glow.setColor(QColor(theme["glow"]))
        if MACRO_DATA[self.name]["active"]: 
            self.glow.setBlurRadius(25)
            self.setStyleSheet(f"background: {theme['bg']}; border-radius: 12px; border: none;")
        else: 
            self.glow.setBlurRadius(0)
            self.setStyleSheet("background: #0e0e0e; border-radius: 12px; border: none;")
...
class NeroClient(QMainWindow):
    def __init__(self):
        ...
        s_lay = QVBoxLayout(sidebar); self.brand = QLabel("NeroM | Global"); s_lay.addWidget(self.brand)
        ...
        info = QVBoxLayout(); 
        u = QLabel("User"); u.setStyleSheet("color:white; font-weight:bold; font-size:10px; border:none;")
        st = QLabel("🟢 Active"); st.setStyleSheet("color:#00ff88; font-size:8px; border:none;")
        ...
        bot = QHBoxLayout(); bot.addStretch(); self.conn = QLabel("CONNECTED"); self.conn.setStyleSheet("color:#00ff88; font-weight:bold; font-size:10px; margin: 10px 20px; border:none;"); bot.addWidget(self.conn); c_lay.addLayout(bot)
    ...
    def init_pages(self):
        ...
        p5 = QWidget(); p5l = QVBoxLayout(p5); box = QFrame(); box.setObjectName("SettingsBox"); bl = QVBoxLayout(box); bl.addWidget(QLabel("Menu Keybind", styleSheet="color:#eee; border:none;"))
...
class ThemeCard(QFrame):
    ...
    def apply_t(self, name): 
        SETTINGS["current_theme"] = name
        comm.theme_changed.emit()
